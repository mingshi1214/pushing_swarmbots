...
def generate_launch_description():
    ...
    nodelist = []
    poselist = [[1.5, 1.35], [2.5, 1.35], [0.85, 2], [3.15, 2], [1.5, 2.65], [2.5, 2.65]]#[[1.5, 1.37], [2.5, 1.37], [0.87, 2], [3.13, 2], [1.5, 2.63], [2.5, 2.63]]
    for i in range(0, 6):
        ...
        nodelist.append(DeclareLaunchArgument(f'goal_x_{robot_name}', default_value = '[3.0, 1.0, 2.0]', description = 'goal (x)'))
        # nodelist.append(DeclareLaunchArgument(f'goal_y_{robot_name}', default_value = str(poselist[i][1]+3.0), description = 'goal (y)'))
        nodelist.append(DeclareLaunchArgument(f'goal_y_{robot_name}', default_value = '[3.0, 1.0, 2.0]', description = 'goal (y)'))
        nodelist.append(DeclareLaunchArgument(f'goal_t_{robot_name}', default_value = '[1.57, 1.57, 1.57]', description = 'goal (t)'))
